...
bool model_ai_system_init(void) {
    ...
    // 如果使用虚拟环境，重建sys.path：使用pyvenv.cfg查找正确的Python路径
    if (found_venv) {
        ...
        snprintf(setup_venv_cmd, sizeof(setup_venv_cmd),
                 ...
                 "        venv_dynload = os.path.join(py_ver_dir, 'lib-dynload')\n"
                 "        if os.path.exists(venv_dynload):\n"
                 "            venv_paths.append(venv_dynload)\n"
                 ...
                 "            base_python_dirs = sorted(glob.glob(os.path.join(lib_dir, 'python*')))\n"
                 "            for base_py_dir in base_python_dirs:\n"
                 "                if os.path.isdir(base_py_dir) and 'site-packages' not in base_py_dir:\n"
                 "                    if venv_version and venv_version in base_py_dir:\n"
                 ...
                 "        base_python_dirs = sorted(glob.glob(os.path.join(base_lib, 'python*')))\n"
                 "        for base_py_dir in base_python_dirs:\n"
                 "            if os.path.isdir(base_py_dir) and 'site-packages' not in base_py_dir:\n"
                 "                if venv_version and venv_version in base_py_dir:\n"
                 "                    base_stdlib.append(base_py_dir)\n"
