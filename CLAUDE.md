...
这是一个基于 C+SDL2 游戏引擎和 Python+PyTorch 深度强化学习的坦克大战 AI 训练系统。
- **C 代码**: 游戏引擎核心,提供高性能的游戏逻辑和渲染
- **Python 代码**: DQN 深度强化学习训练,使用 PyTorch+CUDA 加速
- **混合架构**: C 通过 ctypes 暴露接口给 Python,实现训练循环
...
**玩家对战模式** (src/main.c):
- 自动扫描 `saved_models/`, `saved_models/checkpoints/`
- 按修改时间排序,显示最新的模型
- 用户选择模型后加载
